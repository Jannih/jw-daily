...
final settingsRepositoryProvider = Provider<void>((ref) {
  ...
  final pushNotificationsEnabled =
      sharedPreferences.getBool(_pushNotificationsEnabledKey) ?? false;
  final notificationTimeHour =
      sharedPreferences.getInt(_notificationTimeHourKey) ?? 20;
  final notificationTimeMinute =
      sharedPreferences.getInt(_notificationTimeMinuteKey) ?? 0;
  ...
  ref.read(settingsProvider.notifier).init(Settings(
        ...
        notificationTime: TimeOfDay(
            hour: notificationTimeHour, minute: notificationTimeMinute),
      ));
  ...
  ref.listen(
      ...
      (previousSettings, currentSettings) =>
          currentSettings.whenData((settings) {
            ...
            sharedPreferences.setBool(_pushNotificationsEnabledKey,
                settings.pushNotificationsEnabled);
            sharedPreferences.setInt(
                _notificationTimeHourKey, settings.notificationTime.hour);
            sharedPreferences.setInt(
                _notificationTimeMinuteKey, settings.notificationTime.minute);
          }));
